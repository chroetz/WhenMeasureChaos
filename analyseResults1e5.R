files <- dir("./data1e5", pattern = "^WhenMeasureChaos_\\d+_.*\\.RDS")
paths <- paste0("./data1e5/", files)
...
dim(projDstArray) <- c(2, 100, 100, length(projDstLst))
...
data |> 
  filter(sd == 0.2, deltaT <= 8) |> 
  mutate(
    low = meanMSE - 2*sqrt(totalVar / 1e4),
    high = meanMSE + 2*sqrt(totalVar / 1e4)
  ) |> 
